...
import { handleWish } from "../../api/wishList";
  ...
  const ListingCard=({card})=>{
    ...
    const isLiked = wishList?.find((item) => item?._id === card._id);
    ...
    return (
      <Card className="h-[600px] w-[450px] shadow-lg p-2">
        ...
        <CardBody>
          <div className="mb-3 flex items-center justify-between">
            <Typography variant="h5" color="blue-gray" className="font-medium">
              {card.aptSuite},{card.city},{card.state}
            </Typography>
          ...
          <div className="group mt-8 inline-flex flex-wrap items-center gap-3">
            {card?.amenities?.slice(0,5).map((item,index)=>(
              <Tooltip
              key={index}
               content={item}>
              <span className="flex">
            ...
            {card.amenities.length>5 &&
            <Tooltip content="And +20 more">
              <span className="cursor-pointer rounded-full border border-gray-900/5 bg-gray-900/5 p-3 text-gray-900 transition-colors hover:border-gray-900/10 hover:bg-gray-900/10 hover:!opacity-100 group-hover:opacity-70">
                <p>+{card.amenities.length-5}</p>
              </span>
            </Tooltip>
              }   
          </div>
        ...
        <CardFooter className="pt-3">
          <Button size="lg" fullWidth={true} onClick={()=>navigate(`/properties/${card._id}`)}>
            Details
          </Button>
        </CardFooter>
      </Card>
